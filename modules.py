...
class BasicBlock3D(nn.Module):
    ...
    def forward(self, x):
        ...
        return out
# Add this function before the CALayersP class definition
def PartPooling(x, with_max_pool=True):
    """
    Part-based pooling function for XGait model
    
    Args:
        x: Input tensor of shape [n*s, c, h, w]
        with_max_pool: Whether to use max pooling (otherwise average pooling)
    
    Returns:
        Pooled tensor of shape [n*s, c]
    """
    ...
    # Apply pooling across spatial dimensions
    return pool(x, (1, 1)).view(x.size(0), -1)
